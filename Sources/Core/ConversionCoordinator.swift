...
final class ConversionCoordinator: ObservableObject {
    ...
    private let engagement = EngagementManager.shared
    ...
    func add(urls: [URL], securityScoped: Bool = false) {
        ...
        if !pro.hasAccess {
            if expandedFromFolder {
                note("Whole-folder conversion is a Premium feature. You can still choose one file and convert it free.")
                showProGate = true
                return
            }
            if convertible.count > 1 {
                enqueue([convertible[0]], securityScoped: securityScoped)
                note("Free converts one file at a time. Try Premium for the remaining \(convertible.count - 1) files.")
                return
            }
        }
    ...
    private func enqueue(_ urls: [URL], securityScoped: Bool) {
        let auto = settings.autoConvertOnDrop && pro.isPro
        for url in urls {
    ...
    private func run(_ job: ConversionJob) async {
        ...
        do {
            ...
            job.status = .done
            completedCount += 1
            engagement.recordSuccessfulConversion(isPremium: pro.isPro)
    ...
    func startAllReady() {
        guard pro.isPro || readyCount <= 1 else {
            showProGate = true
            return
        }
