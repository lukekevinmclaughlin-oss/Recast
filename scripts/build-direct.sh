...
# Build, sign, notarize and verify the fully unlocked website edition. The
# Release configuration remains the StoreKit-backed App Store edition.
set -euo pipefail

repo_dir="$(cd "$(dirname "$0")/.." && pwd)"
build_root="${RECAST_DIRECT_BUILD_ROOT:-/private/tmp/macossoftware-recast-direct}"
derived_data="$build_root/DerivedData"
source_dir="$build_root/src"
stage_dir="$build_root/stage"
output_dir="${RECAST_DIRECT_OUTPUT_DIR:-/Users/$USER/Desktop/MacOSSoftware Direct Releases/Recast}"
...
[[ "$build_root" == /private/tmp/macossoftware-recast-direct* || -n "${RECAST_DIRECT_BUILD_ROOT:-}" ]] || {
...
app_path="$derived_data/Build/Products/Direct/Recast.app"
[[ -d "$app_path" ]] || { echo "Recast.app was not produced" >&2; exit 1; }
bundle_id="$(defaults read "$app_path/Contents/Info" CFBundleIdentifier)"
[[ "$bundle_id" == "com.lukemclaughlin.recast.direct" ]] || {
  echo "Unexpected direct bundle identifier: $bundle_id" >&2
  exit 1
}

executable="$app_path/Contents/MacOS/Recast"
if otool -L "$executable" | grep -q 'StoreKit.framework'; then
  echo "Refusing to package a website build linked to StoreKit" >&2
  exit 1
fi
if rg -a -q 'Start Free Trial|Restore Purchases|Manage Subscription|Auto-renews|Try Premium' "$executable"; then
...
dmg_path="$output_dir/Recast-${version}-direct.dmg"
rm -f "$dmg_path"
hdiutil create -volname "Recast Direct" -srcfolder "$stage_dir" -ov -format UDZO "$dmg_path"
